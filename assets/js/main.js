document.addEventListener("DOMContentLoaded", (event) => {
    ...
    // Apply organization settings from admin to public site
    try {
        const adminSettings = JSON.parse(localStorage.getItem('adminSettings') || '{}');
        const org = adminSettings.organization;
        if (org) {
            const infos = document.querySelectorAll('.ul-footer-top-contact-infos .ul-footer-top-contact-info');
            if (infos && infos.length >= 3) {
                // Address block (1st)
                const addrEl = infos[0].querySelector('.ul-footer-top-contact-info-address');
                if (addrEl) {
                    addrEl.innerHTML = `${org.address || ''}`;
                }
                // Email block (2nd)
                const emailEl = infos[1].querySelector('.ul-footer-top-contact-info-address a');
                if (emailEl && org.email) {
                    emailEl.textContent = org.email;
                    emailEl.setAttribute('href', `mailto:${org.email}`);
                }
                // Phone block (3rd)
                const phoneEl = infos[2].querySelector('.ul-footer-top-contact-info-address a');
                if (phoneEl && org.phone) {
                    phoneEl.textContent = org.phone;
                    const tel = org.phone.replace(/[^+\d]/g, '');
                    phoneEl.setAttribute('href', `tel:${tel}`);
                }
            }
        }
    } catch (e) {
        console.warn('Org settings not applied:', e);
    }
